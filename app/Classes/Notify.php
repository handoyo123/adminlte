...
class Notify
{
    ...
    public static function getData($sendFor, $sendData): array
    {
        $data = [];
    ...
    public static function getDataArray($data): array
    {
        return [
     ...
     * @throws ApiException
     */
    public static function isAbleToSendMail($sendFor, $dataArray): array
    {
        ...
        if ($mailSetting && $mailSetting->status && $mailSetting->verified) {
            ...
            if ($sendMailSettings && $sendMailSettings->credentials && in_array($sendFor, $sendMailSettings->credentials)) {
                ...
                $title = $notificationSetting->credentials ? $notificationSetting->credentials['title'] : '';
                $content = $notificationSetting->credentials ? $notificationSetting->credentials['content'] : '';
