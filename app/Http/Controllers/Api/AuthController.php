...
class AuthController extends ApiBaseController
{
    ...
    public function getPurchaseSales()
    {
        ...
        $periodDates = $this->getPeriodDates($startDate, $endDate);
        $datesArray = [];
        $purchasesArray = [];
        $salesArray = [];
        ...
        foreach ($periodDates as $periodDate) {
            ...
            if (isset($allPurchases[$currentDate]) || isset($sales[$currentDate])) {
                ...
                $purchasesArray[] = $allPurchases[$currentDate] ?? 0;
                $salesArray[] = $sales[$currentDate] ?? 0;
            }
        }
    ...
    public function getPaymentChartData()
    {
        ...
        foreach ($periodDates as $periodDate) {
            ...
            if (isset($allSentPayments[$currentDate]) || isset($allReceivedPayments[$currentDate])) {
                $datesArray[] = $currentDate;
                $sentPaymentsArray[] = $allSentPayments[$currentDate] ?? 0;
                $receivedPaymentsArray[] = $allReceivedPayments[$currentDate] ?? 0;
            }
        }
     ...
     * @return CarbonPeriod
     */
    public function getPeriodDates($startDate, $endDate): CarbonPeriod
    {
